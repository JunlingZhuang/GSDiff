...
const builders = {
  ...
  // Thin screen on a short wall bracket, mounted high (screen faces +z into the room).
  patient_monitor() {
    return [
      box(0.25, 0.25, 0.35, 0, 1.45, -0.18, ACCENT), // bracket
      box(1.5, 1.0, 0.14, 0, 1.5, 0.05, BODY), // screen body
      box(1.3, 0.8, 0.03, 0, 1.5, 0.13, ACCENT), // screen face
    ];
  },
  ...
  // Seat + backrest + arms + legs. Back is at -z.
  visitor_chair() {
    return [
      box(1.7, 0.22, 1.6, 0, 1.35, 0.1, BODY), // seat
      box(1.7, 1.5, 0.2, 0, 2.1, -0.8, BODY), // backrest
  ...
  // Carcass + counter overhang + door panels + pulls.
  casework() {
    const meshes = [
      box(5.9, 2.9, 1.9, 0, 1.45, 0, BODY), // carcass
      box(6.2, 0.15, 2.2, 0, 3.0, 0.15, ACCENT), // counter top
    ];
    for (const x of [-1.9, 0, 1.9]) {
      meshes.push(box(1.7, 2.4, 0.06, x, 1.4, 0.98, ACCENT)); // door panel
      meshes.push(box(0.09, 0.35, 0.09, x + 0.7, 1.9, 1.0, BODY)); // pull
    }
    return meshes;
  },
  // Cabinet + counter + basin recess (boolean-free inset) + faucet.
  handwash_sink() {
    return [
      box(1.9, 2.85, 1.65, 0, 1.425, 0, BODY), // cabinet
      box(2.1, 0.15, 1.85, 0, 3.0, 0, ACCENT), // counter
      box(1.1, 0.45, 0.9, 0, 2.72, 0.05, ACCENT), // basin recess
      box(0.1, 0.55, 0.1, 0, 3.3, -0.6, BODY), // faucet post
