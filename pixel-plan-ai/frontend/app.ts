...
type Samples = Record<string, Program>;
...
const ui = {
  sample: element<HTMLSelectElement>("sampleSelect"),
  editor: element<HTMLTextAreaElement>("programEditor"),
  prompt: element<HTMLTextAreaElement>("promptInput"),
  width: element<HTMLInputElement>("gridWidth"),
  height: element<HTMLInputElement>("gridHeight"),
  generate: element<HTMLButtonElement>("generateButton"),
  status: element<HTMLDivElement>("requestStatus"),
  provider: element<HTMLSpanElement>("providerStatus"),
  jsonStatus: element<HTMLDivElement>("jsonStatus"),
  canvas: element<HTMLCanvasElement>("planCanvas"),
  empty: element<HTMLDivElement>("canvasEmpty"),
  tooltip: element<HTMLDivElement>("canvasTooltip"),
  title: element<HTMLHeadingElement>("outputTitle"),
  subtitle: element<HTMLParagraphElement>("outputSubtitle"),
  score: element<HTMLDivElement>("scoreValue"),
  roomCount: element<HTMLElement>("roomCount"),
  legend: element<HTMLDivElement>("legendList"),
  validationCount: element<HTMLElement>("validationCount"),
  validation: element<HTMLDivElement>("validationList"),
  doorCount: element<HTMLElement>("doorCount"),
  doors: element<HTMLDivElement>("doorList"),
  code: element<HTMLTextAreaElement>("codeOutput"),
  source: element<HTMLElement>("codeSource"),
  metadata: element<HTMLDivElement>("metadataGrid"),
  areas: element<HTMLTableSectionElement>("areaTable"),
  png: element<HTMLButtonElement>("downloadPng"),
  json: element<HTMLButtonElement>("downloadJson"),
  copy: element<HTMLButtonElement>("copyCode"),
  gridWidthLabel: element<HTMLElement>("gridWidthLabel"),
  generateLabel: element<HTMLElement>("generateLabel"),
  generateHint: element<HTMLElement>("generateHint"),
  generationOverlay: element<HTMLDivElement>("generationOverlay"),
  generationPhase: element<HTMLHeadingElement>("generationPhase"),
  generationElapsed: element<HTMLElement>("generationElapsed"),
  stopGeneration: element<HTMLButtonElement>("stopGeneration"),
  iterationList: element<HTMLDivElement>("iterationList"),
  runCode: element<HTMLButtonElement>("runCode"),
  inspectCode: element<HTMLButtonElement>("inspectCode"),
  fixCode: element<HTMLButtonElement>("fixCode"),
...
async function initialize(): Promise<void> {
  ...
  if (!samplesResponse.ok || !healthResponse.ok) throw new Error("The backend did not initialize correctly.");
  state.samples = (await samplesResponse.json()) as Samples;
...
function validateEditor(): Program | null {
  try {
    const value = JSON.parse(ui.editor.value) as Program;
...
type AgentAction = "generate" | "run" | "inspect" | "fix" | "revise";

function setAgentButtons(enabled: boolean): void {
...
function actionCopy(action: AgentAction): { phase: string; status: string } {
  const values: Record<AgentAction, { phase: string; status: string }> = {
...
function renderGenerationProgress(progress: GenerationProgress, program: Program): void {
  ...
  if (!preview.plan || !preview.validation) return;
...
async function pollGenerationProgress(
  ...
  while (!signal.aborted && !isFinished()) {
    ...
    try {
      ...
      renderGenerationProgress((await response.json()) as GenerationProgress, program);
    } catch (error) {
...
async function requestAgentAction(action: AgentAction): Promise<void> {
  ...
  try {
    ...
    if (action === "inspect") switchView("data");
    ...
    if (state.activeRequestId === requestId) {
      if (state.generationTimer !== null) window.clearInterval(state.generationTimer);
      ...
      if (!state.result && !state.previewPlan) ui.empty.hidden = false;
    }
  }
}
...
function renderResult(result: GenerationResult): void {
...
function renderCanvas(plan: Plan): void {
...
function renderLegend(plan: Plan): void {
...
function renderValidation(validation: Validation): void {
...
function renderDoors(doors: Door[]): void {
...
function renderData(result: GenerationResult): void {
...
function renderIterations(iterations: GenerationIteration[]): void {
...
function switchView(view: string): void {
  ...
  document.querySelectorAll<HTMLElement>(".view-panel").forEach((panel) => panel.classList.remove("active"));
  const target = document.getElementById(`${view}View`);
  if (target) target.classList.add("active");
}

function download(name: string, type: string, content: string | Blob): void {
...
element<HTMLButtonElement>("formatJson").addEventListener("click", () => {
...
ui.png.addEventListener("click", () => {
  ui.canvas.toBlob((blob) => { if (blob) download("pixel-plan.png", "image/png", blob); }, "image/png");
});
ui.json.addEventListener("click", () => {
  if (state.result) download("pixel-plan.json", "application/json", JSON.stringify(state.result, null, 2));
});
...
ui.canvas.addEventListener("mousemove", (event: MouseEvent) => {
  ...
  const stage = ui.canvas.closest<HTMLElement>(".canvas-stage")?.getBoundingClientRect();
  if (stage) {
