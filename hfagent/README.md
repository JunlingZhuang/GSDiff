...
# geometry-only real-plan trace (faithful walls + arc-confirmed doors, untyped rooms):
python -m hfagent.evaluate --program hospital-tower-floor --structure-mode linework
# repeat runs (independent full pipeline runs per program, dirs <program>-r1..-rN):
python -m hfagent.evaluate --structure-mode linework --repeat 4 -p clinic-small
...
`linework` mode is geometry-only (rooms come out untyped, ids `r1..rN`) and runs a
single round. Every run writes the fixed trace artifacts instead of colour-block
images: `walls_overlay.png` (faithful walls on the source), `doors_overlay.png`
(walls + detected swing arcs), `recon.png` (bridged walls + door arcs),
`recon_post.png` (post-processed wall graph) and `rooms_colorful.png` (closed room
polygons in distinct deterministic colours, doors as white gaps).

Useful flags: `--program` / `-p` / `--only <name>[,<name>]` (run named program(s)),
`--n <N>` (first N — quick smoke only, **not** a substitute for the full run),
`--repeat <N>` (N independent full runs per program, dirs `<program>-r1..-rN`),
`--structure-mode colorblock|json|linework|direct_colorblock` (`colorblock` remains
the default; `linework` traces the real drawing deterministically — faithful walls,
arc-confirmed doors, untyped rooms; `json` keeps the VLM-structure pipeline
available), `--out <dir>` (override the timestamped dir).
