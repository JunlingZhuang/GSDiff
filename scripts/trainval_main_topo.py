...
'''This is the script for training the first-stage node generation model under topology constraints.'''
...
while step < total_steps:
    ...
    '''The training set topology is given as a constraint, and the topology is passed through the graph embedding network to obtain the embedding of the topology nodes as a constraint.
Note that there is padding, which is masked to all 0.'''
    ...
    # 12 bits bitwise regression, the maximum value is 12
    ...
    if step % interval == 0 or step == 1000:
        ...
        for batch_count in tqdm(range(batch_numbers)):
            ...
            bb_semantics_val_batch = bb_semantics_val_batch.to(device) # Low-dimensional room semantics with padding to 8
            bb_adjacency_matrix_val_batch = bb_adjacency_matrix_val_batch.to(device) # Adjacency Matrix of topology graph
            bb_semantics_padding_mask_val_batch = bb_semantics_padding_mask_val_batch.to(device) # Padding mask of topology graph
            bb_global_matrix_val_batch = bb_global_matrix_val_batch.to(device) # global_attn_matrix of the topology graph
        ...
        print('The validation set prediction image is rendered')
        '''calculate FID, KID. '''
