...
from gsdiff.heterhouse_56_31 import *
...
'''This is a script to train the second-stage edge prediction model under topological constraints.'''
...
while step < total_steps:
    ...
    # corners_padding_mask(bs, 53, 1) 1 up and 0 down,
    ...
    # Implementation: Same as missing at random
...
Note that there is padding, which is masked to all 0.'''
    ...
    if step % interval == 0:
        ...
        for val_count in tqdm(range(val_number)):
            ...
            bb_semantics_val_batch = bb_semantics_val_batch.to(device)
            bb_adjacency_matrix_val_batch = bb_adjacency_matrix_val_batch.to(device)
            bb_semantics_padding_mask_val_batch = bb_semantics_padding_mask_val_batch.to(device)
            bb_global_matrix_val_batch = bb_global_matrix_val_batch.to(device)
