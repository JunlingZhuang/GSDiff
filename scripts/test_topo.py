...
'''Data'''
dataset_test = RPlanGEdgeSemanSimplified_80('test')
...
print('The graph embedding network has been frozen')

model_path_EdgeModel = 'outputs/topo-edge/model_stage2_best_076000.pt'
...
# DDPM
test_metrics = []
model_path_CDDPMs = ['outputs/topo-node-ddpm/' + fn for fn in os.listdir('outputs/topo-node-ddpm') if 'model' in fn and '.pt' in fn]
for model_path_CDDPM in model_path_CDDPMs:
    if int(model_path_CDDPM[5 + len('outputs/topo-node-ddpm/'):8 + len('outputs/topo-node-ddpm/')]) % 1000 == 100: # model1000000.pt
        ...
        for batch_count in tqdm(range(batch_numbers)):
            bb_semantics_test_batch, bb_adjacency_matrix_test_batch, bb_semantics_padding_mask_test_batch, bb_global_matrix_test_batch, bb_room_number_test_batch, corners_withsemantics_0_test_batch, global_attn_matrix_test_batch, corners_padding_mask_test_batch = next(dataloader_test_iter)





            bb_semantics_test_batch = bb_semantics_test_batch.to(device) # Low-dimensional room semantics, padded to 8
            bb_adjacency_matrix_test_batch = bb_adjacency_matrix_test_batch.to(device) # Adjacency Matrix of Topological Graph
            bb_semantics_padding_mask_test_batch = bb_semantics_padding_mask_test_batch.to(device) # Padding mask
            bb_global_matrix_test_batch = bb_global_matrix_test_batch.to(device)# global_attn_matrix


            '''The training set topology graph is given as a constraint, 
            and the topology graph is passed through the graph embedding network to obtain the embedding of the topology graph nodes as a constraint.'''
            ...
            bb_semantics_embedding_test_batch = bb_semantics_embedding_test_batch * bb_semantics_padding_mask_test_batch  # (bs, 8, 256)
                        ...
                        for i, component_edges in enumerate(grouped_edge_sets):
                            ...
                            for component_edges_t in component_edges:
                                for pnt in component_edges_t:
                                    component_edges_vertices.append(pnt)
                            # print(f"Connected Component {i}: {component_edges}")
                            # Align y for each edge set, find the average (rounded) of y for each edge set, and then update the coordinates in cleaned_polygons
        ...
        '''calculate FID, KID. 
        This rendering method is different, is in the validation set, and generates a different number of samples, which is not the result reported in the paper. 
        The results reported in the paper are obtained by calculating 757 samples that overlap with House-GAN++ and HouseDiffusion 5 times through the test script.'''
