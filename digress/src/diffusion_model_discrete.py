...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.val_counter = 0
        self.history_path = "training_history.csv"
        self.history_fallback_warned = False
    ...
    def on_train_epoch_end(self) -> None:
        ...
        x_ce = self._metric_to_float(to_log['train_epoch/x_CE'])
        e_ce = self._metric_to_float(to_log['train_epoch/E_CE'])
        y_ce = self._metric_to_float(to_log['train_epoch/y_CE'])
        if x_ce < 0 or e_ce < 0:
            return
        lambda_x = 1.0
        lambda_e = float(self.cfg.model.lambda_train[0])
        lambda_y = float(self.cfg.model.lambda_train[1])
        train_total = lambda_x * x_ce + lambda_e * e_ce
        y_text = "n/a"
        if y_ce >= 0 and lambda_y != 0:
            train_total += lambda_y * y_ce
            y_text = f"{y_ce:.3f}"
        elif y_ce >= 0:
            y_text = f"{y_ce:.3f}"
        self.log("train/epoch_weighted_loss", train_total, sync_dist=True)
        self.print(
            f"Epoch {display_epoch}: train_loss={train_total:.3f} "
            f"(node_type_loss={x_ce:.3f} + {lambda_e:g}*edge_type_loss={e_ce:.3f}) "
        ...
        self._append_history({
            ...
            "split": "train",
            "train_weighted": train_total,
            "train_x_ce": x_ce,
            "train_e_ce": e_ce,
            "train_y_ce": "" if y_text == "n/a" else y_text,
            "train_seconds": time.time() - self.start_epoch_time,
        })
        epoch_at_metrics, epoch_bond_metrics = self.train_metrics.log_epoch_metrics()
        if epoch_at_metrics is not None or epoch_bond_metrics is not None:
            self.print(f"Epoch {display_epoch}: {epoch_at_metrics} -- {epoch_bond_metrics}")
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.log("val_epoch_NLL", val_nll, sync_dist=True)
        self.log("display_epoch", float(display_epoch), sync_dist=True)

        if val_nll < self.best_val_nll:
            self.best_val_nll = val_nll
        self.print(
            f"Epoch {display_epoch}: val_nll={val_nll:.4f} "
            f"(X_KL={metrics[1]:.3f}, E_KL={metrics[2]:.3f}, "
            f"best_val_nll={self.best_val_nll:.4f})\n"
        )
        self._append_history({
            "epoch": display_epoch,
            "global_step": self.global_step,
            "split": "val",
            "val_nll": self._metric_to_float(val_nll),
            "val_x_kl": self._metric_to_float(metrics[1]),
            "val_e_kl": self._metric_to_float(metrics[2]),
            "best_val_nll": self._metric_to_float(self.best_val_nll),
        })
    ...
    def _append_history(self, row):
        ...
        try:
            self._write_history_row(self.history_path, columns, row)
        except PermissionError:
            fallback_path = f"training_history_{os.getpid()}.csv"
            if not self.history_fallback_warned:
                self.print(
                    f"Could not write {self.history_path}; it may be open in another program. "
                    f"Continuing with {fallback_path}."
                )
                self.history_fallback_warned = True
            self.history_path = fallback_path
            self._write_history_row(self.history_path, columns, row)
        self._plot_training_history()

    @staticmethod
    def _write_history_row(path, columns, row):
        rows = []
        if os.path.exists(path):
            with open(path, newline="") as f:
                reader = csv.DictReader(f)
                for existing_row in reader:
                    if not DiscreteDenoisingDiffusion._is_valid_history_row(existing_row):
                        continue
                    if (existing_row.get("epoch") == str(row.get("epoch")) and
                            existing_row.get("split") == str(row.get("split"))):
                        continue
                    rows.append({column: existing_row.get(column, "") for column in columns})

        rows.append({column: row.get(column, "") for column in columns})
        rows.sort(key=lambda item: (
            int(float(item["epoch"])) if item.get("epoch") else -1,
            0 if item.get("split") == "train" else 1,
        ))
    ...
    @staticmethod
    def _is_valid_history_row(row):
        split = row.get("split")
        try:
            int(float(row.get("epoch", "")))
        except (TypeError, ValueError):
            return False

        if split == "train":
            try:
                return (float(row.get("train_weighted", "")) >= 0 and
                        float(row.get("train_x_ce", "")) >= 0 and
                        float(row.get("train_e_ce", "")) >= 0)
            except (TypeError, ValueError):
                return False

        if split == "val":
            try:
                return float(row.get("val_nll", "")) >= 0
            except (TypeError, ValueError):
                return False
    ...
    def _plot_training_history(self):
        ...
        try:
            with open(self.history_path, newline="") as f:
                ...
                rows = [
                    ...
                    if DiscreteDenoisingDiffusion._is_valid_history_row(row)
                ]
        except OSError:
            return
        ...
        def series(split, key):
            ...
            for history_row in rows:
                if history_row.get("split") != split:
                    continue
                value = history_row.get(key, "")
                if value == "":
                    continue
                xs.append(float(history_row["epoch"]))
                ys.append(float(value))
            return xs, ys

        plot_specs = [
            ("train", "train_weighted", "Train Weighted Loss"),
            ("train", "train_x_ce", "Train Node CE"),
            ("train", "train_e_ce", "Train Edge CE"),
            ("val", "val_nll", "Validation NLL"),
            ("val", "val_x_kl", "Validation Node KL"),
            ("val", "val_e_kl", "Validation Edge KL"),
        ]

        fig, axes = plt.subplots(2, 3, figsize=(13, 7))
        for ax, (split, key, title) in zip(axes.flatten(), plot_specs):
        ...
        try:
            ...
            fig.savefig(f"training_curves_{os.getpid()}.png", dpi=160)
        finally:
            plt.close(fig)
            ...
            for i in range(num_molecules):
                ...
                if not os.path.exists(result_path):
                    ...
                    _ = self.visualization_tools.visualize_chain(result_path,
                                                                 chain_X[:, i, :].numpy(),
                                                                 chain_E[:, i, :].numpy())
                self.print('{}/{} chains complete'.format(i + 1, num_molecules))
