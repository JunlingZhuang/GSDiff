...
def main(cfg: DictConfig):
    ...
    matmul_precision = cfg.general.get("matmul_precision", None)
    if matmul_precision is not None:
        torch.set_float32_matmul_precision(matmul_precision)

    if dataset_config["name"] == 'rplan':
        from datasets.rplan_dataset import RPlanDataModule, RPlanDatasetInfos
        from analysis.rplan_utils import RPlanSamplingMetrics, RPlanVisualization

        datamodule = RPlanDataModule(cfg)
        dataset_infos = RPlanDatasetInfos(datamodule, dataset_config)
        train_metrics = TrainAbstractMetricsDiscrete() if cfg.model.type == 'discrete' else TrainAbstractMetrics()
        sampling_metrics = RPlanSamplingMetrics(datamodule)
        visualization_tools = RPlanVisualization(dataset_infos)
    ...
    configured_strategy = cfg.general.get("strategy", "auto")
    ...
    if not cfg.general.test_only:
        ...
        if getattr(trainer, "interrupted", False):
            print("Training interrupted. Skipping automatic test.")
            return
